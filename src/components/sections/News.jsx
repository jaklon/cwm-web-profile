...
function ImageArea({ src, title }) {
  ...
  return (
    <div className="w-full h-full flex flex-col items-center justify-center bg-gradient-to-br from-blue-500 via-cyan-500 to-blue-600 min-h-[200px]">
      ...
      <p className="text-xs font-semibold text-white/80 uppercase tracking-widest">Speaker Event</p>
    </div>
  );
}
  ...
  return (
    <motion.div
      ...
      className="
        ...
        bg-white border border-slate-200 border-l-4 border-l-blue-500
    ...
    >
      ...
      {/* Content */}
      <div className="flex-1 p-7 lg:p-8 flex flex-col gap-4 min-w-0">
        ...
        {/* Title + subtitle */}
        <div>
          <h3 className="text-2xl font-bold font-display text-slate-900 leading-tight mb-1">
            {item.title}
          </h3>
          <p className="text-sm italic text-slate-600">{item.subtitle}</p>
        </div>
        ...
        <div className="flex items-center gap-3 p-3.5 rounded-xl bg-slate-50 border border-slate-200">
          ...
          <div className="min-w-0">
            <p className="text-sm font-bold text-slate-900 leading-none mb-0.5">{item.speaker}</p>
            <p className="text-xs text-slate-500">{item.speakerRole}</p>
          </div>
        </div>
        ...
        <div className="flex items-center gap-2">
          ...
          <span className="text-xs text-slate-500">Diselenggarakan oleh</span>
          <span className="text-xs font-semibold text-slate-700">{item.organizer}</span>
        </div>
        ...
        <ul className="space-y-2">
          {item.highlights.map((h) => (
            <li key={h} className="flex items-start gap-2.5">
              ...
              <span className="text-xs text-slate-600 leading-relaxed">{h}</span>
            </li>
          ))}
        </ul>

        {/* Bottom info bar */}
        <div className="flex flex-wrap items-center gap-4 pt-3 border-t border-slate-100 mt-auto">
        ...
        <AnimatedSection direction="up" delay={0.15}>
          ...
          ">
            ...
            <p className="text-sm font-medium text-slate-400">
              More achievements and news coming soon...
            </p>
            <p className="text-xs text-slate-300">
              Follow our activities and milestones here
            </p>
          </div>
        </AnimatedSection>
